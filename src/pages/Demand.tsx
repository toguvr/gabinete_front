						...
						<View style={styles.table}>
							...
							{selectFilter === 'responsible' ? (
								<TextPDF style={styles.tableSubTitle}>
									Lista de Demandas -{' '}
									{responsibles.find((responsible) => responsible.label === responsibleFilterField)
										?.label || ''}
								</TextPDF>
							) : (
							...
							<View style={styles.tableContainer}>
								...
								{pageItems.map((item: TaskPropsDTO) => (
									...
									>
										...
										<TextPDF style={styles.cell}>{item?.voter.cellphone}</TextPDF>
										<TextPDF style={styles.address}>
											{item?.voter?.zip
												? `${item?.voter?.street ? item?.voter?.street + ',' : ''}
                              ${
																item?.voter?.address_number ? item?.voter?.address_number + ',' : ''
															}
                              ${item?.voter?.neighborhood ? item?.voter?.neighborhood + ',' : ''}
                              ${item?.voter?.complement ? item?.voter?.complement + ',' : ''}
                              ${item?.voter?.city ? item?.voter?.city + ',' : ''}
                              ${item?.voter?.state ? item?.voter?.state + ',' : ''}`
												: '-'}
										</TextPDF>
									</View>
								))}
	...
	return (
		<HeaderSideBar>
			...
			<Flex justifyContent="space-between" flexDir={['column', 'row']}>
				<Flex gap={['12px', '24px']}>
					...
					{selectFilter === 'responsible' ? (
						...
						<Input
							...
							name="filterField"
							placeholder="Buscar"
							value={selectFilter === 'deadline' ? filterFieldDateMask : filterField}
							mb="24px"
							onChange={(e) => {
				...
				<PDFDownloadLink document={<MyDocument />} fileName={`demandas-${office?.name}.pdf`}>
					<Button w={['160px', '280px']} leftIcon={<Icon fontSize="20" as={IoPrintOutline} />}>
						Imprimir
					</Button>
				</PDFDownloadLink>
			</Flex>
								...
								return (
									<Tr key={task.id} whiteSpace="nowrap">
										...
										<Td
											...
											py="4px"
										>
											{task?.deadline ? getFormatDate(new Date(task?.deadline), 'dd/MM/yyyy') : '-'}
										</Td>
										<Td
											color="gray.600"
											fontSize="14px"
